...
# ---------- 配色(深色高对比) ----------
BG          = "#10131f"   # 页面背景:深蓝黑
CARD        = "#191e30"   # 卡片:比背景亮一层
CARD2       = "#242b45"   # 次级底/输入框:再亮一层
BORDER      = "#333b5c"   # 边框
ACCENT      = "#7c8cf8"   # 主色:亮靛蓝
ACCENT_HOV  = "#96a4ff"
GREEN       = "#3ddc97"   # 成功绿
GREEN_DIM   = "#1d2e29"   # 高亮卡片底
AMBER       = "#fbbf24"   # 倒计时琥珀
RED         = "#f87171"   # 停止/删除红
TEXT        = "#f4f6fd"   # 主文字:近白,与背景强对比
TEXT2       = "#b9c1d8"   # 次要文字:浅灰蓝
TEXT3       = "#8590ab"   # 提示文字:中灰蓝
HL_TEXT     = "#a7f3d0"   # 高亮文字
WHITE       = "#ffffff"
...
class PositionCard(ctk.CTkFrame):
    ...
    def highlight(self, on):
        if on:
            ...
            self.coord.configure(text_color=HL_TEXT)
        else:
...
class MultiClickerApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text="循环点击 / 连点器", font=("Microsoft YaHei UI", 12),
                     text_color=TEXT3).pack(side="left", padx=(8, 0), pady=(7, 0))
        ...
        self.cursor_label = ctk.CTkLabel(header, text="(0, 0)", fg_color=CARD2,
    ...
    def _row_box(self, parent, label_text, widget_factory, padx=16, pady=(4, 8)):
        ...
        ctk.CTkLabel(wrap, text=label_text, font=("Microsoft YaHei UI", 12),
                     text_color=TEXT2).pack(side="left", padx=(12, 0), pady=8)
    ...
    def _build_loop_tab(self, tab):
        ...
        ctk.CTkLabel(right, text="点击类型", font=("Microsoft YaHei UI", 12),
                     text_color=TEXT2).pack(padx=16, pady=(10, 4), anchor="w")
    ...
    def _seg(self, parent, values, var):
        return ctk.CTkSegmentedButton(
            ...
            font=("Microsoft YaHei UI", 12), fg_color=CARD2,
    ...
    def _build_spam_tab(self, tab):
        ...
        self.fixed_label = ctk.CTkLabel(
            ...
            font=("Microsoft YaHei UI", 12),
            text_color=TEXT if self.fixed_pos else TEXT3)
        ...
        ctk.CTkLabel(card, text="提示:连点器立即开始,无倒计时;运行中按 ESC 或 F9 停止",
                     font=("Microsoft YaHei UI", 11), text_color=TEXT3).pack(pady=(0, 16))
    ...
    def _set_loop_btn_countdown(self, on):
        if on:
            self.loop_toggle_btn.configure(fg_color=AMBER, hover_color=AMBER,
                                           text_color="#1a1a1a")
        else:
